...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    ...
    @Query(value = "SELECT r.start_date, r.end_date, ro.number, ro.type, ro.id FROM reservations r, rooms ro, reservations_rooms rm WHERE r.guest_id = ?1 AND r.id = rm.reservation_id AND rm.room_id = ro.id;", nativeQuery = true)
